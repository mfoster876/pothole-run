...
// `maxW` clamps each line so long entries condense instead of running off a narrow stage.
function section(ctx, x, y, title, titleColor, items, itemColor, emptyText, maxW, lineH = 19) {
  ctx.fillStyle = titleColor; ctx.font = '700 15px "Courier New", monospace';
  ctx.fillText(title, x, y);
  ctx.font = '500 13px "Courier New", monospace';
  if (items.length) {
    ctx.fillStyle = itemColor;
    items.forEach((t, i) => ctx.fillText('• ' + t, x, y + 20 + i * lineH, maxW));
    return y + 20 + items.length * lineH + 14;
  }
  ctx.fillStyle = '#7a8a7e';
  ctx.fillText(emptyText, x, y + 20, maxW);
...
export function render(ctx, { characterId, W, H }) {
  ...
  const compact = W < 700;   // narrow portrait stage — the two columns can't fit side by side
  ctx.fillStyle = '#f0c020'; ctx.font = '700 32px "Courier New", monospace';
  ctx.fillText('LEGEND', W / 2, H * 0.06);

  // Driver portrait + the risk/reward note
  renderPortrait(ctx, ch.id, W * 0.5, compact ? H * 0.145 : H * 0.19, 76);
  ctx.fillStyle = '#cbe7cf'; ctx.font = 'italic 500 14px "Courier New", monospace';
  ctx.fillText(note, W / 2, compact ? H * 0.225 : H * 0.31, W * 0.94);

  ctx.textAlign = 'left';
  if (compact) {
    // Single stacked column: edge → weakness → grab → dodge → watch.
    const x = W * 0.07, maxW = W * 0.88, lh = 17;
    let y = H * 0.265;
    y = section(ctx, x, y, '⚡ UNFAIR EDGE', '#e0a52a', perks, '#e8d8b0', '(none)', maxW, lh);
    y = section(ctx, x, y, '⚠ WEAKNESS', '#c0382c', cons, '#f0b8b0', '(none)', maxW, lh);
    y = section(ctx, x, y, '✦ PICK UP', '#3fae54', good.map(g => g.label), '#e8efe6', '(none)', maxW, lh);
    y = section(ctx, x, y, '✕ AVOID', '#c0382c', bad.map(b => b.label), '#f0b8b0', '(nothing — drive free)', maxW, lh);
    section(ctx, x, y, 'PEOPLE TO WATCH', '#e0a52a', people, '#e8d8b0', '(none)', maxW, lh);
  } else {
    ...
    let yL = topY;
    yL = section(ctx, colL, yL, '⚡ UNFAIR EDGE', '#e0a52a', perks, '#e8d8b0', '(none)', maxW);
    section(ctx, colL, yL, '✦ PICK UP', '#3fae54', good.map(g => g.label), '#e8efe6', '(none)', maxW);

    let yR = topY;
    yR = section(ctx, colR, yR, '⚠ WEAKNESS', '#c0382c', cons, '#f0b8b0', '(none)', maxW);
    yR = section(ctx, colR, yR, '✕ AVOID', '#c0382c', bad.map(b => b.label), '#f0b8b0', '(nothing — drive free)', maxW);
    section(ctx, colR, yR, 'PEOPLE TO WATCH', '#e0a52a', people, '#e8d8b0', '(none)', maxW);
  }
